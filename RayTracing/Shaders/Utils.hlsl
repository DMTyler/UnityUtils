...
float3 BlinnPhong(float3 normal, float3 viewDir, float3 lightDir, float3 lightColor, float3 ambientColor, float3 diffuseColor, float3 specularColor, float shininess)
{
    // 规范化输入向量
    ...
    float3 diffuse = NdotL * diffuseColor * lightColor;
    ...
    float3 specularComponent = specular * specularColor * lightColor;

    // 计算最终颜色
    float3 finalColor = ambientColor + diffuse + specularComponent;
